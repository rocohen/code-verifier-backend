...
// - Get Katas By Level
export const getKataByLevel = async (
  level: string
): Promise<any | undefined> => {
  try {
    const kataModel = kataEntity();
...
// - Rate a Kata
export const rateKata = async (
  id: string,
  userValoration: number
): Promise<any | undefined> => {
  try {
    const kataModel = kataEntity();
    // Rate a Kata
    ...
    const avgValoration =
      (kata.valoration * kata.peopleWhoRated + userValoration) /
      (kata.peopleWhoRated + 1);
